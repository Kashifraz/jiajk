    ...
    <div>
        ...
        <div class="py-5  gap-4 max-w-7xl mx-auto sm:px-6 lg:px-8 ">
            <div class=" p-4 sm:p-8 bg-white shadow sm:rounded-lg mb-4">
                <div class="md:col-span-3">
                    <h2 class="text-lg font-medium text-gray-900 mb-4">
                        Showing Member information for <b class="capitalize">{{$formB->user->name}}</b>
                    </h2>
            ...
            <div class=" p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="md:col-span-3">
                    <h2 class="text-lg font-medium text-gray-900 mb-4">
                        Showing Form B for <b class="capitalize">{{$formB->user->name}}</b>
                    </h2>
            ...
            <!-- comments form aproval and disapproval -->
            @if($formB->sg_approval == null || $formB->dpd_approval == null || $formB->pd_approval == null )
            @if (($formB->dpd_approval != null && $formB->dpd_approval != "disapprove" && Auth::user()->can('second approval forma'))
            ||($formB->dpd_approval == null && Auth::user()->can('first approval forma'))
            || ($formB->sg_approval != null && $formB->sg_approval != "disapprove" && Auth::user()->can('third approval formb')))
            <div class="p-4 mt-4 sm:p-8 bg-white shadow sm:rounded-lg">
                ...
                if (Auth::user()->can('first approval forma'))
                    echo route('form.b.approval.dpd', $formB->id);
                else if (Auth::user()->can('second approval forma'))
                    echo route('form.b.approval.sg', $formB->id);
                else if (Auth::user()->can('third approval formb'))
                    echo route('form.b.approval.dp', $formB->id);
            ...
            <div class="p-4 mt-4 sm:p-8 bg-white shadow sm:rounded-lg  antialiased">
                <div class="max-w-full mx-auto px-4">
                    ...
                    <!-- deputy president comments -->
                    @php
                    $dpd = User::find($formB->dpd_id);
                    @endphp
                    <article class="p-6 text-base bg-white rounded-lg">
                        <footer class="flex justify-start items-center mb-2">
                            <div class="flex items-center">
                                ...
                                <p class="text-sm text-gray-600 ">{{date("F d, Y", strtotime($formB->dpd_approval_date))}}
                                    @if ($formB->dpd_approval == "approve")
                                    <span class="bg-green-200 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded mx-3">Approved</span>
                                    @elseif($formB->dpd_approval == "disapprove")
                                    <span class="bg-red-200 text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded mx-3">Disapproved</span>
                                    @endif
                                </p>
                            </div>
                        </footer>
                        <p class="text-gray-500">{{$formB->dpd_comment}}</p>
                    </article>
                    <!-- secretery general comments -->
                    @if($formB->sg_approval != null)
                    @php
                    $sg = User::find($formB->sg_id);
                    @endphp
                    <article class="p-6 text-base bg-white rounded-lg">
                        <footer class="flex justify-start items-center mb-2">
                            <div class="flex items-center">
                                ...
                                <p class="text-sm text-gray-600 ">{{date("F d, Y", strtotime($formB->sg_approval_date))}}
                                    @if ($formB->sg_approval == "approve")
                                    <span class="bg-green-200 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded mx-3">Approved</span>
                                    @elseif($formB->sg_approval == "disapprove")
                                    <span class="bg-red-200 text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded mx-3">Disapproved</span>
                                    @endif
                                </p>
                            </div>
                        </footer>
                        <p class="text-gray-500">{{$formB->sg_comment}}</p>
                    </article>
                    <!-- President comments -->
                    @if($formB->pd_approval != null)
                    @php
                    $pd = User::find($formB->pd_id);
                    @endphp
                    <article class="p-6 text-base bg-white rounded-lg">
                        <footer class="flex justify-start items-center mb-2">
                            <div class="flex items-center">
                                <p class="inline-flex items-center mr-3 text-sm text-gray-900  font-semibold">
                                    @if ($pd->profile)
                                    <img class="mr-2 w-6 h-6 rounded-full" src="{{ asset('uploads/'.$pd->profile) }}" alt="Michael Gough">
                                    @endif
                                    <span class="capitalize">{{"Mr/Ms ".$pd->name}} - (PD)</span>
                                </p>
                                <p class="text-sm text-gray-600 ">{{date("F d, Y", strtotime($formB->pd_approval_date))}}
                                    @if ($formB->pd_approval == "approve")
                                    <span class="bg-green-200 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded mx-3">Approved</span>
                                    @elseif($formB->pd_approval == "disapprove")
                                    <span class="bg-red-200 text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded mx-3">Disapproved</span>
                                    @endif
                                </p>
                            </div>
                        </footer>
                        <p class="text-gray-500">{{$formB->pd_comment}}</p>
                    </article>
                    @endif
                    @endif
                </div>
